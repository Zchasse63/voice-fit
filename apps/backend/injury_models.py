...
class InjuryAnalyzeRequest(BaseModel):
    ...
    recent_exercises: Optional[List[str]] = Field(default=None, description="Recent exercises performed")
    training_context: Optional[str] = Field(default=None, description="Additional training context")
...
class InjuryLogRequest(BaseModel):
    ...
    body_part: str = Field(..., description="Injured body part (e.g., 'shoulder', 'lower_back')")
    severity: SeverityEnum = Field(..., description="Severity: 'mild', 'moderate', or 'severe'")
    description: Optional[str] = Field(default=None, description="Injury description")
    status: Optional[StatusEnum] = Field(default=StatusEnum.active, description="Status: 'active', 'recovering', or 'resolved'")
...
class InjuryCheckInRequest(BaseModel):
    ...
    rom_quality: str = Field(..., description="Range of motion: 'better', 'same', or 'worse'")
    activity_tolerance: str = Field(..., description="Activity tolerance: 'improving', 'plateau', or 'declining'")
...
class InjuryAnalyzeResponse(BaseModel):
    ...
    severity: Optional[str] = Field(default=None, description="Severity: 'mild', 'moderate', or 'severe'")
    injury_type: Optional[str] = Field(default=None, description="Type of injury (e.g., 'strain', 'sprain')")
    description: str = Field(..., description="Human-readable description of the injury")
    red_flags: List[str] = Field(default_factory=list, description="Red flag symptoms requiring medical attention")
    recommendations: List[str] = Field(default_factory=list, description="Recovery recommendations")
    requires_medical_attention: bool = Field(..., description="Whether immediate medical attention is required")
...
class InjuryLogResponse(BaseModel):
    ...
    resolved_at: Optional[datetime] = Field(default=None, description="When injury was resolved")
    last_check_in_at: Optional[datetime] = Field(default=None, description="Last check-in timestamp")
...
class ActiveInjuryResponse(BaseModel):
    ...
    injuries: List[InjuryLogResponse] = Field(..., description="List of active injuries")
    count: int = Field(..., description="Total count of active injuries")
    needs_check_in: List[str] = Field(default_factory=list, description="Injury IDs that need weekly check-in")


class RecoveryCheckInResponse(BaseModel):
    """Response model for recovery check-in"""
    progress_score: float = Field(..., ge=0.0, le=1.0, description="Recovery progress score (0.0-1.0)")
    status: str = Field(..., description="Recovery status: 'improving', 'plateau', 'worsening', or 'resolved'")
    recommendation: str = Field(..., description="Recommendation based on recovery status")
    requires_medical_attention: bool = Field(..., description="Whether medical consultation is recommended")
    days_in_recovery: int = Field(..., description="Number of days since injury was reported")
...
class ExerciseSubstitutionRequest(BaseModel):
    ...
    exercise_name: str = Field(..., description="Name of the exercise to find substitutes for")
    injured_body_part: Optional[str] = Field(None, description="Body part to protect: shoulder, lower_back, knee, elbow, hip")
    min_similarity_score: Optional[float] = Field(0.60, ge=0.0, le=1.0, description="Minimum similarity threshold (0.0-1.0)")
    max_results: Optional[int] = Field(5, ge=1, le=20, description="Maximum number of substitutes to return")
    equipment_filter: Optional[List[str]] = Field(None, description="Filter by available equipment")
    difficulty_filter: Optional[List[str]] = Field(None, description="Filter by difficulty level")
...
class ExerciseSubstitution(BaseModel):
    ...
    primary_muscles: str = Field(..., description="Primary muscles worked (comma-separated)")
...
class ExerciseSubstitutionResponse(BaseModel):
    ...
    substitutes: List[ExerciseSubstitution] = Field(..., description="List of substitute exercises")
...
class ExplanationContext(BaseModel):
    ...
    pain_level: Optional[int] = Field(None, ge=0, le=10, description="Current pain level (0-10)")
...
class ExplanationSections(BaseModel):
    ...
    why_recommended: str = Field(..., description="Why this substitution is recommended")
    scientific_evidence: str = Field(..., description="Scientific evidence from research")
    ...
    recovery_context: Optional[str] = Field(None, description="Recovery-specific guidance")
...
class ExerciseExplanationResponse(BaseModel):
    ...
    sections: ExplanationSections = Field(..., description="Individual explanation sections")
