...
class BadgeService:
    ...
    def _add_volume_badges(self):
        ...
        milestones = [
            (50_000, "50K"), (100_000, "100K"), (250_000, "250K"), (500_000, "500K"),
            (1_000_000, "1M"), (2_500_000, "2.5M"), (5_000_000, "5M"), (10_000_000, "10M")
        ]
        for lbs, label in milestones:
            self.badges[f"volume_milestone_{label.lower().replace('.', '_')}"] = BadgeDefinition(
    ...
    def _add_plate_milestone_badges(self):
        ...
        plates = [
            ("bench", 1, 135), ("bench", 2, 225), ("bench", 3, 315),
            ("squat", 2, 225), ("squat", 3, 315), ("squat", 4, 405),
            ("deadlift", 3, 315), ("deadlift", 4, 405), ("deadlift", 5, 495)
        ]
    ...
    def _add_run_single_distance_badges(self):
        ...
        distances = [
            ("5k", "5K Finisher", 3.1), ("10k", "10K Finisher", 6.2),
            ("15k", "15K Finisher", 9.3), ("half", "Half Marathon", 13.1),
            ("20mi", "20 Miler", 20.0), ("marathon", "Marathon", 26.2),
    ...
    def _add_run_elevation_badges(self):
        ...
        elevations = [
            (500, "Hill Climber"), (1000, "Mountain Goat"),
            (2500, "Peak Bagger"), (5000, "Summit Seeker")
        ]
    ...
    def _add_hybrid_badges(self):
        ...
        cross_training = [(25, "Hybrid Athlete"), (100, "Iron Runner"), (500, "Ultimate Athlete")]
    ...
    async def check_workout_badges(self, user_id: str) -> List[str]:
        ...
        volume_milestones = [
            (50_000, "50k"), (100_000, "100k"), (250_000, "250k"), (500_000, "500k"),
            (1_000_000, "1m"), (2_500_000, "2_5m"), (5_000_000, "5m"), (10_000_000, "10m")
        ]
    ...
    async def check_run_badges(self, user_id: str, run_data: Dict[str, Any]) -> List[str]:
        """
        ...
        # Check single run distance badges
        run_distance = run_data.get('distance_miles', 0)
        distance_badges = [
            (3.1, "5k"), (6.2, "10k"), (9.3, "15k"), (13.1, "half"),
            (20.0, "20mi"), (26.2, "marathon"), (31.0, "ultra")
        ]
    ...
    async def _check_run_speed_badges(self, user_id: str, run_data: Dict[str, Any]) -> List[str]:
        """Check for speed achievement badges (5K, 10K, Mile)"""
        newly_earned = []

        distance = run_data.get('distance_miles', 0)
    ...
    async def _check_run_pace_badges(self, user_id: str, run_data: Dict[str, Any]) -> List[str]:
    ...
    async def _check_run_elevation_badges(self, user_id: str, run_data: Dict[str, Any]) -> List[str]:
    ...
    async def _check_run_weather_badges(self, user_id: str, run_data: Dict[str, Any]) -> List[str]:
    ...
    async def _get_pr_count(self, user_id: str) -> int:
        ...
        result = self.supabase.table('pr_history')\
            .select('id', count='exact')\
            .eq('user_id', user_id)\
            .execute()
        return result.count or 0
    ...
    async def _get_run_count(self, user_id: str) -> int:
        ...
        result = self.supabase.table('runs')\
            .select('id', count='exact')\
            .eq('user_id', user_id)\
            .execute()
        return result.count or 0
    ...
    async def _get_total_run_distance(self, user_id: str) -> float:
        """Get total distance run in miles"""
        result = self.supabase.table('runs')\
        ...
        for run in result.data or []:
            total += run.get('distance_miles', 0) or 0
    ...
    async def _get_current_workout_streak(self, user_id: str) -> int:
        ...
        for workout in result.data:
            workout_date = datetime.fromisoformat(workout['start_time'].replace('Z', '+00:00')).date()
    ...
    async def _get_current_run_streak(self, user_id: str) -> int:
        ...
        for run in result.data:
            run_date = datetime.fromisoformat(run['date']).date() if isinstance(run['date'], str) else run['date']
    ...
    async def _get_weekly_consistency(self, user_id: str) -> int:
        ...
        for year, week in sorted_weeks:
            if (year, week) == (expected_year, expected_week):
                ...
                temp_date = date.fromisocalendar(expected_year, expected_week, 1) - timedelta(days=7)
    ...
    async def get_badge_progress(self, user_id: str) -> Dict[str, Any]:
        ...
        """
        # Get earned badges
        earned_result = self.supabase.table('user_badges')\
            .select('badge_type')\
            .eq('user_id', user_id)\
            .execute()
