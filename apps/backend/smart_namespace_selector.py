...
class SmartNamespaceSelector:
    ...
    def select_namespaces(self, questionnaire: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        """
        ...
        if 'strength' in goal or 'powerlifting' in goal:
            selected['strength-and-hypertrophy'] = {'top_k': 4, 'content_types': ['programming', 'principle'], 'priority': 1}
            selected['strength-training'] = {'top_k': 3, 'content_types': ['programming'], 'priority': 1}
            selected['powerlifting-programs'] = {'top_k': 3, 'content_types': ['programming'], 'priority': 1}
            selected['periodization-theory'] = {'top_k': 2, 'content_types': ['principle'], 'priority': 2}
            selected['autoregulation'] = {'top_k': 2, 'content_types': ['programming'], 'priority': 2}
        
        if 'hypertrophy' in goal or 'muscle' in goal or 'bodybuilding' in goal:
            selected['hypertrophy'] = {'top_k': 4, 'content_types': ['programming', 'principle'], 'priority': 1}
            selected['strength-and-hypertrophy'] = {'top_k': 3, 'content_types': ['programming'], 'priority': 1}
            selected['muscle-specific-programming'] = {'top_k': 3, 'content_types': ['programming'], 'priority': 1}
            selected['volume-management'] = {'top_k': 2, 'content_types': ['programming'], 'priority': 2}
        
        if 'general' in goal or 'fitness' in goal:
            selected['fitness-assessment'] = {'top_k': 3, 'content_types': ['principle'], 'priority': 1}
            selected['beginner-fundamentals'] = {'top_k': 3, 'content_types': ['principle', 'programming'], 'priority': 1}
            selected['general'] = {'top_k': 2, 'content_types': ['concept'], 'priority': 3}
        
        # Experience-based selection
        if 'beginner' in experience:
            selected['beginner-fundamentals'] = {'top_k': 4, 'content_types': ['principle', 'programming'], 'priority': 1}
            selected['movement-patterns'] = {'top_k': 2, 'content_types': ['principle'], 'priority': 2}
            selected['adherence'] = {'top_k': 2, 'content_types': ['concept'], 'priority': 2}
        ...
        if 'advanced' in experience or 'intermediate' in experience:
            ...
            selected['periodization-concepts'] = {'top_k': 2, 'content_types': ['principle'], 'priority': 2}
            selected['fatigue-management'] = {'top_k': 2, 'content_types': ['programming'], 'priority': 2}
        
        # Injury-based selection
        if injuries and len(injuries) > 0:
            selected['injury-management'] = {'top_k': 4, 'content_types': ['recovery', 'programming'], 'priority': 1}
            selected['injury-prevention'] = {'top_k': 3, 'content_types': ['principle'], 'priority': 1}
            selected['exercise-selection'] = {'top_k': 2, 'content_types': ['programming'], 'priority': 2}
            selected['equipment-substitution'] = {'top_k': 2, 'content_types': ['programming'], 'priority': 2}
        
        # Weak point selection
        if weak_points and len(weak_points) > 0:
            selected['sticking-points'] = {'top_k': 3, 'content_types': ['programming'], 'priority': 1}
            
            # Add lift-specific technique namespaces
            for weak_point in weak_points:
                if 'squat' in weak_point.lower():
                    selected['squat-technique'] = {'top_k': 3, 'content_types': ['principle', 'exercise'], 'priority': 1}
        
        # Equipment-based selection
        if equipment and 'bodyweight' in str(equipment).lower():
            selected['calisthenics'] = {'top_k': 3, 'content_types': ['programming'], 'priority': 1}
            selected['bodyweight-training'] = {'top_k': 2, 'content_types': ['programming'], 'priority': 2}
        
        # Always include recovery and nutrition
        selected['recovery-and-performance'] = {'top_k': 3, 'content_types': ['recovery', 'principle'], 'priority': 2}
        selected['nutrition-and-supplementation'] = {'top_k': 2, 'content_types': ['principle'], 'priority': 2}
        
        # Add programming essentials
        selected['programming'] = {'top_k': 3, 'content_types': ['programming', 'principle'], 'priority': 1}
        selected['programming-logic'] = {'top_k': 2, 'content_types': ['programming'], 'priority': 2}
        selected['program-templates'] = {'top_k': 2, 'content_types': ['programming'], 'priority': 2}
        
        return selected
    
    def retrieve_knowledge(self, query: str, namespace_config: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        sorted_namespaces = sorted(
            namespace_config.items(),
            key=lambda x: (x[1]['priority'], -x[1]['top_k'])
        )
    ...
    def _get_semantic_weight(self, namespace: str, query: str) -> float:
        ...
        conceptual_keywords = [
            'periodization', 'theory', 'principle', 'concept',
            'programming-logic', 'adherence', 'mindset'
        ]
        ...
        exercise_keywords = [
            'technique', 'exercise', 'movement', 'squat',
            'bench', 'deadlift', 'overhead'
        ]
    ...
    def get_rag_context(self, questionnaire: Dict[str, Any], max_chunks: int = 50) -> str:
        """
        ...
        context_parts.append(f"Retrieved {len(chunks)} knowledge chunks from {len(set(c['namespace'] for c in chunks))} specialized namespaces.\n")
...
if __name__ == "__main__":
    ...
    for ns, config in sorted(namespaces.items(), key=lambda x: x[1]['priority']):
        print(f"  [{config['priority']}] {ns}: {config['top_k']} chunks, types: {config['content_types']}")
