...
class CoachQuestionRequest(BaseModel):
    ...
    question: str = Field(..., description="User's question to the AI Coach", min_length=1, max_length=500)
        ...
        json_schema_extra = {
            "example": {
                ...
                "conversation_history": [
                    ...
                    {"role": "assistant", "content": "6-12 reps is optimal for hypertrophy."}
                ]
            }
        }
...
class ProgramGenerationRequest(BaseModel):
    ...
    questionnaire: Dict[str, Any] = Field(..., description="User's training questionnaire data")
...
class RunningParseRequest(BaseModel):
    ...
    latitude: Optional[float] = Field(None, description="Starting latitude for weather data")
    longitude: Optional[float] = Field(None, description="Starting longitude for weather data")
...
class RunningParseResponse(BaseModel):
    ...
    elevation_data: Optional[Dict[str, Any]] = Field(None, description="Elevation and GAP data")
...
class RunningAnalyzeResponse(BaseModel):
    ...
    elevation_analysis: Dict[str, Any] = Field(..., description="Elevation and GAP analysis")
    performance_insights: str = Field(..., description="AI-generated performance insights")
    recommendations: List[str] = Field(default_factory=list, description="Training recommendations")
    comparison_to_recent_runs: Optional[Dict[str, Any]] = Field(None, description="Comparison to recent similar runs")
...
class VolumeAnalyticsResponse(BaseModel):
    ...
    volume_trend: Dict[str, Any] = Field(..., description="Volume trend over multiple weeks")
...
class FatigueAnalyticsResponse(BaseModel):
    ...
    current_fatigue: Dict[str, Any] = Field(..., description="Current fatigue assessment")
    fatigue_history: Dict[str, Any] = Field(..., description="Fatigue history over multiple weeks")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "fatigue_history": {
                    "weeks": [
                        {"week_start": "2024-01-01", "fatigue_level": "low", "fatigue_score": 20.0}
                    ]
                }
            }
        }
...
class DeloadRecommendationResponse(BaseModel):
    ...
    deload_type: Optional[str] = Field(None, description="Type of deload: programmed or auto_regulation")
    ...
    recommendation: Optional[str] = Field(None, description="Specific deload recommendation")
...
class WorkoutInsightsResponse(BaseModel):
    ...
    volume_analysis: Dict[str, Any] = Field(..., description="Volume analysis by muscle group")
    intensity_analysis: Dict[str, Any] = Field(..., description="Intensity and RPE analysis")
    muscle_group_balance: Dict[str, Any] = Field(..., description="Muscle group balance assessment")
    fatigue_indicators: Dict[str, Any] = Field(..., description="Fatigue and recovery indicators")
    performance_insights: str = Field(..., description="AI-generated performance insights")
    recommendations: List[str] = Field(default_factory=list, description="Training recommendations")
    comparison_to_recent_workouts: Optional[Dict[str, Any]] = Field(None, description="Comparison to recent similar workouts")
...
class AdherenceCheckInRequest(BaseModel):
    ...
    response_type: str = Field(..., description="Response type: injury, time_constraint, equipment, motivation, fine, change_program")
    injury_details: Optional[Dict[str, Any]] = Field(None, description="Injury details if response_type is 'injury'")
    details: Optional[str] = Field(None, description="Additional details about the response")
...
class AdherenceCheckInResponse(BaseModel):
    ...
    adjustment_plan: Optional[Dict[str, Any]] = Field(None, description="Suggested adjustment plan")
...
class AdherenceReportResponse(BaseModel):
    ...
    user_id: str
    check_date: str
    adherence: Dict[str, Any] = Field(..., description="Adherence data (actual vs target volume)")
    flags: Dict[str, Any] = Field(..., description="Flag status (created, updated, resolved, alerts)")
    imbalance_risks: List[Dict[str, Any]] = Field(default_factory=list, description="Detected imbalance risks")
    needs_user_action: bool = Field(..., description="Whether user needs to take action")
...
class ChatClassifyRequest(BaseModel):
    ...
    conversation_history: Optional[List[Dict[str, str]]] = Field(None, description="Recent conversation history for context")
...
class ChatClassifyResponse(BaseModel):
    ...
    message_type: str = Field(..., description="Classified message type: workout_log, question, general, onboarding")
    confidence: float = Field(..., description="Classification confidence (0.0-1.0)")
    reasoning: Optional[str] = Field(None, description="Explanation of classification")
    suggested_action: Optional[str] = Field(None, description="Suggested next action for the UI")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "confidence": 0.95,
                "reasoning": "Message contains weight and reps, indicating a workout set log",
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "conversation_history": [
                    {"role": "assistant", "content": "Welcome! Let's get started. How long have you been training?"},
...
class BadgeUnlockRequest(BaseModel):
    ...
    user_id: str = Field(..., description="User ID")
    badge_type: str = Field(..., description="Badge type to unlock (e.g., 'workout_count_10', 'pr_count_5')")
