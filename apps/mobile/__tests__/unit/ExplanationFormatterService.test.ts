...
describe("ExplanationFormatterService", () => {
  const mockSubstitution: ExerciseSubstitution = {
    ...
    notes:
      "EMG shows 13-25% higher pec activation with dumbbells; greater ROM (can descend below chest); independent arm movement corrects imbalances; 6RM load ~40kg dumbbells vs 88kg barbell",
  };

  describe("formatExplanation", () => {
    it("should format basic explanation without context", () => {
      const result =
        ExplanationFormatterService.formatExplanation(mockSubstitution);
    ...
    it("should include injury-specific context when provided", () => {
      const result = ExplanationFormatterService.formatExplanation(
        mockSubstitution,
        {
      ...
      expect(result.sections.why_recommended).toContain(
    ...
    it("should format similarity score correctly for different ranges", () => {
      ...
      const result1 =
        ExplanationFormatterService.formatExplanation(highSimilarity);
      ...
      const result2 =
        ExplanationFormatterService.formatExplanation(moderateSimilarity);
    ...
    it("should parse scientific notes into bullet points", () => {
      ...
      expect(result.sections.scientific_evidence).toContain(
        "EMG shows 13-25% higher pec activation",
      );
    ...
    it("should combine all sections into full explanation", () => {
      ...
      expect(result.explanation).toContain(
        "Why Dumbbell Bench Press is recommended",
      );
  ...
  describe("Edge cases", () => {
    it("should handle substitution with no reduced stress area", () => {
      ...
      expect(result.sections.why_recommended).not.toContain(
        "Reduces Shoulder Stress",
      );
    });
    ...
    it("should handle empty notes field", () => {
      ...
      expect(result.sections.scientific_evidence).toContain(
        "No additional scientific notes",
      );
    });
    ...
    it("should handle very low similarity scores", () => {
      ...
      const result =
        ExplanationFormatterService.formatExplanation(lowSimilarity);
    ...
    it("should handle high pain levels", () => {
      ...
      expect(result.sections.recovery_context).toContain(
